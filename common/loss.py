...
def mpjpe(predicted, target,keepBatch=False):
    """
    ...
    if keepBatch:
        return torch.mean(torch.norm(predicted - target, dim=len(target.shape)-1),dim=-1)
    else:
...
def p_mpjpe(predicted, target,keepBatch=False):
    """
...
def n_mpjpe(predicted, target,keepBatch=False):
    """
    ...
    return mpjpe(scale * predicted, target,keepBatch)

def mean_velocity_error(predicted, target,keepBatch=False):
    """
