...
h36m_24_skeleton = Skeleton(parents=[-1,0,1,2,2,3,4,1,1,7,8,9,10,0,0,13,14,15,16,17,18,17,18,1],
...
class Human36m_24Dataset(MocapDataset):
    def __init__(self, path3d, path2d, remove_static_joints=True, chunk_length=1, augment=False, pad=121, causal_shift=0, is_train=False):
        super().__init__(fps=50, skeleton=copy.deepcopy(h36m_24_skeleton))
        self.chunk_length = chunk_length
        self.augment = augment
        self.pad = pad
        self.causal_shift = causal_shift
        self.is_train = is_train
        self.train_subjects = ["S1", "S5", "S6", "S7", "S8","setting1", "setting3",  "setting4",  "setting5", "setting71",  "setting72",  "setting81",  "setting82",  "setting91",  "setting92","setting101",  "setting102","aerobicgymnastics","badminton","martialarts","rhythmicgymnastics","rumba"]
        self.test_subjects = ["S9", "S11","setting2","setting6","sanda"]
        # self.test_subjects = ["S9", "S11","setting1",  "setting2",  "setting3",  "setting4",  "setting5",  "setting6",  "setting71",  "setting72",  "setting81",  "setting82",  "setting91",  "setting92","setting101",  "setting102", "aerobic_gymnastics","badminton","martial_arts","rhythmic_gymnastics","rumba","sanda"]
        # self.test_subjects = ["aerobicgymnastics","badminton","martialarts","rhythmicgymnastics","rumba","sanda"]
    ...
    def process3DPose(self):
        ...
        for subject, actions in data_3d.items():
            ...
            for action_name, positions in actions.items():
                ...
                self._data_3d[subject][action_name] = {'positions': positions_update, 'cameras': self._cameras[subject], 'positions_3d': positions_3d}
    ...
    def normalize2DPose(self):
        for subject in self._data_3d.keys():
            ...
            action_list = list(self._data_3d[subject].keys())
            action_list.sort()
            for action in action_list:
                ...
                for cam_idx in range(len(self._data_2d[subject][action])):
                    ...
                    self.subject_action_camIndex.append([subject, action, str(cam_idx)])
    ...
    def getDataFromPair(self, index):
        ...
        if pad_left_2d != 0 or pad_right_2d != 0:
            group_3d = np.pad(copy.deepcopy(seq_3d[low_2d:high_2d]), ((pad_left_2d, pad_right_2d), (0, 0), (0, 0)), 'edge')
        else:
            group_3d = copy.deepcopy(seq_3d[low_2d:high_2d])
        ...
        if self.is_train:
            ...
            seq_2d_rand = copy.deepcopy(seq_2d[rand_no])
            seq_3d_rand = copy.deepcopy(seq_3d[rand_no])
            if flip:
                # Flip 2D keypoints
                seq_2d_rand[:, :, 0] *= -1
                seq_2d_rand[:, self.kps_left + self.kps_right] = seq_2d_rand[:, self.kps_right + self.kps_left]
                seq_3d_rand[:, :, 0] *= -1
                seq_3d_rand[:, self.joints_left + self.joints_right] = seq_3d_rand[:, self.joints_right + self.joints_left]

            return subject_action_camIndex, group_cam.astype(np.float32), group_3d.astype(np.float32), group_2d.astype(
                np.float32), seq_3d_rand.astype(np.float32), seq_2d_rand.astype(np.float32)
        else:
            return subject_action_camIndex, group_cam.astype(np.float32), group_3d.astype(np.float32), group_2d.astype(np.float32)
